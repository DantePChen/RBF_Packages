...
MStatus initializePlugin(MObject pluginObj)
{
	...
	*/
	...
	if (!status)
	{
		MGlobal::displayError("Failed to register the RBF_Solver_Node" + status.errorString());
		return(status);
	}

	// load the WeightMultiplyNode
	...
	if (!status)
	{
		MGlobal::displayError("Failed to register the WeightMultiplyNode" + status.errorString());
		return(status);
	}
