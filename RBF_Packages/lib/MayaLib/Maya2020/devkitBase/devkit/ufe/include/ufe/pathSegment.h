...
UFE_NS_DEF {
    class Path;
//! \brief A segment of a path within a single run-time.
/*!
A path segment is part of a path to an object (either containment or 3D
hierarchy) where all objects on the path segment live in a single run-time.
...
public:
    ...
    /*!
        Creates a PathSegment for the argument run-time.
    
        \param[in] segment   String representation of segment, with separators.
        \param[in] runTimeId ID of run time for this segment.
    ...
    */
    ...
    /*!
    Creates a PathSegment for the argument run time [noexcept].
