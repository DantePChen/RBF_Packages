...
class frameContextTest(omr.MRenderOverride):
    ...
    def renderOperation(self):
        if self.mCurrentOperationIndex >= 0 and self.mCurrentOperationIndex < len(self.mRenderOperations):
            return self.mRenderOperations[list(self.mRenderOperations.keys())[self.mCurrentOperationIndex]]
        else:
            return None
    ...
    def setup(self, panelName ):
        ...
        if frameCtx:
            ...
            ufeCameraPath = frameCtx.getCurrentUfeCameraPath()
            print('> Ufe Camera path: ' + ufeCameraPath)
...
def initializePlugin(obj):
    ...
    mFrameContextInstance = frameContextTest('my_frame_context_test_py')
