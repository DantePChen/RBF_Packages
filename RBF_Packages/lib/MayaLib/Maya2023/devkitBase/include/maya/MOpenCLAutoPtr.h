...
public:
	...
	MAutoCLKernel( MAutoCLKernel&& other );
	MAutoCLKernel& operator=(const MAutoCLKernel& other);
	MAutoCLKernel& operator=(MAutoCLKernel&& other);
...
public:
	...
	MAutoCLMem( MAutoCLMem&& other );
	MAutoCLMem& operator=(const MAutoCLMem& other);
	MAutoCLMem& operator=(MAutoCLMem&& other);
...
// ****************************************************************************
// CLASS DECLARATION (MAutoCLEventList)

//! \ingroup OpenMaya
//! \brief Helper class to keep track of CL events
/*!
    MAutoCLEventList is a simple class to make it easier to manage an event
    wait list for kernels that are queued up.

    Note: this was originally available in Maya 2022 as MGPUEventList in the
    OpenMayaAnim module. MGPUEventList is now a typedef of MAutoCLEventList.
*/
class OPENMAYA_EXPORT MAutoCLEventList
{
public:
    MAutoCLEventList();
