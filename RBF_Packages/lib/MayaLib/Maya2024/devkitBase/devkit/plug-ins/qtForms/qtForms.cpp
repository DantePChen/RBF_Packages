...
,	fSelectObjList(0)
{
	...
	connect(fSelectObjList, qOverload<int>(&QComboBox::currentIndexChanged), this, [&](int index) {
		const QString currentText = fSelectObjList->itemText(index);
		displayObjectDialog(currentText);
	});
...
const MString				qtFormsCmd::fsCommandName("qtForms");
