...
public:
	...
	*/
	...
	static std::unique_ptr< Accessor >	accessorByExtension( const std::string& extension );
	static std::unique_ptr< Accessor >	readFile			( const std::string& fileName,
															  std::string& errors );
...
public:
	...
	/*!
		\fn std::unique_ptr< Accessor > create() const;
		...
		factory.
	*/
	virtual std::unique_ptr< Accessor > create() const = 0;
...
public:
	...
	virtual std::unique_ptr< Accessor > create() const
	{
		return std::unique_ptr< Accessor >( new AccessorType );
	}
