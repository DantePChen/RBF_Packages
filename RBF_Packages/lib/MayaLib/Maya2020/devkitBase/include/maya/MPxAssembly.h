...
//   virtual MStatus      applyEdits		(MObject& target, MItEdits& edits);
   ...
   bool  activating() const;
	...
	// [07/2012]
	// 
	// Internal tracking for more robust solution: MAYA-15346

   MTypeId		typeId		() const /*final*/;
	MString		typeName	() const /*final*/;
	MString		name		() const /*final*/;
	MObject     thisMObject	() const /*final*/;
	MStatus		setExistWithoutInConnections	( bool flag ) /*final*/;
	bool		existWithoutInConnections		( MStatus* ReturnStatus = NULL ) const /*final*/;
	MStatus		setExistWithoutOutConnections	( bool flag ) /*final*/;
	bool		existWithoutOutConnections		( MStatus* ReturnStatus = NULL ) const /*final*/;
...
protected:
	...
	virtual bool		doNotWrite		( MStatus *ReturnStatus = nullptr ) const /*final*/;
END_NO_SCRIPT_SUPPORT:

protected:

	OPENMAYA_DEPRECATED(2019, "Override schedulingType() to set thread safety instead.")
	virtual void		setMPSafe		( bool flag ) /*override final*/;

	OPENMAYA_DEPRECATED(2018, "Call getInitialRep(const MObject &, bool& hasInitialRep, MStatus*) instead.")
