...
template<typename T>
std::vector<PathComponent> TrieNode<T>::childrenComponents() const
{
    std::vector<PathComponent> ret;
    ret.reserve(fChildren.size());
    for (auto c : fChildren)
    {
        ret.emplace_back(c.first);
    }
    return ret;
}
...
bool Trie<T>::containsDescendantHelper(const Path& ancestorPath) const
{
    ...
    typename TrieNode<T>::Ptr trieNode = root();
    for (const PathComponent& c : ancestorPath) {
        typename TrieNode<T>::Ptr child = (*trieNode)[c];
    ...
    // argument path, we simply return true: at this point we've reached
    // either an internal node, which by definition has children, or a leaf
    // node with no descendants, which by definition has data.
    return INCLUDE_ANCESTOR ? true : !trieNode->empty();
}

template<typename T>
bool Trie<T>::containsDescendant(const Path& ancestorPath) const
{
    return containsDescendantHelper<false>(ancestorPath);
}

template<typename T>
bool Trie<T>::containsDescendantInclusive(const Path& ancestorPath) const
{
    return containsDescendantHelper<true>(ancestorPath);
}
