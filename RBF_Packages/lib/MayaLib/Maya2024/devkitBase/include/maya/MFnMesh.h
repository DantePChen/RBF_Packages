...
public:
    ...
    OPENMAYA_ENUM(BoolClassification,
		//! Edge Classification
		kEdgeClassification = 1,
	...
	OPENMAYA_MODIFIED(2023, "classification parameter added")
	...
	OPENMAYA_ENUM( BorderInfo,
		kGeomBorder = -2,	//!< The edge lies on a geometry border
		kUVBorder   = -1,	//!< The edge lies on a UV border
		kSharedUV   = 0,	//!< The faces around the edge share the UVs
		kUnsharedUV = 1		//!< The faces around the edge have different UVs
	);
	OPENMAYA_AVAILABLE(2023)
	...
	MStatus		getMeshShellsIds( MFn::Type compType, 
								...
								unsigned int& nbMeshShells ) const;
