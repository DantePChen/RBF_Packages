...
public:
	...
	*/
    OPENMAYA_ENUM(MRenderOperationType,
		...
		kDataServer, //!< Data server operation
...
public:
	...
	// Requires reset to states of graphics device, if true, the cached states of the graphics device will be invalidated
	virtual bool requiresResetDeviceStates() const { return false; }
};


// ****************************************************************************
// CLASS DECLARATION (MDataServerOperation)
OPENMAYA_AVAILABLE(2024)
//! \ingroup OpenMayaRender
//! \brief Class which defines a dataserver operation.
//! This accesses a set of render items describing the scene, for the purposes
//! of custom rendering.
//! Note that this class is experimental and subject to future changes.
//
class OPENMAYARENDER_EXPORT MDataServerOperation : public MRenderOperation
{
public:

	// MViewportScene decribes a set of MRenderItems that contain renderable data for the scene.
	class MViewportScene
	{
	public:
		enum ChangeFlags {
			// Value flags
			MVS_new = 1 << 0,
			MVS_visible = 1 << 1,
			// Dirty notification flags
			MVS_changedMatrix = 1 << 2,
			MVS_changedGeometry = 1 << 3,
			MVS_changedTopo = 1 << 4,
			MVS_changedVisibility = 1 << 5,
			MVS_changedEffect = 1 << 6,
			MVS_ALL = 0xffff & ~(MVS_visible | MVS_new)
		};

		size_t mCount = 0;
		MRenderItem** mItems = nullptr;

		// same size flags array matches render item indexing
		const unsigned int* mFlags = nullptr;

		size_t mRemovalCount = 0;
		const int* mRemovals = nullptr;
		bool changed() const { return mRemovalCount || mCount; }
	};
