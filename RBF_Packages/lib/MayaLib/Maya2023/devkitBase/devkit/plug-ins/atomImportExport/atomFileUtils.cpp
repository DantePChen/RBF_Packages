...
const char *kWordTangentAutoMix = "automix";
const char *kWordTangentAutoEase = "autoease";
const char *kWordTangentAutoCustom = "autocustom";
...
//
{
	switch (type) {
		...
		case MFnAnimCurve::kTangentAutoMix:
			return (kWordTangentAutoMix);
		case MFnAnimCurve::kTangentAutoEase:
			return (kWordTangentAutoEase);
		case MFnAnimCurve::kTangentAutoCustom:
			return (kWordTangentAutoCustom);
...
//
{
	...
	if (strcmp(type, kWordTangentAutoMix) == 0) {
		return (MFnAnimCurve::kTangentAutoMix);
	}
	if (strcmp(type, kWordTangentAutoEase) == 0) {
		return (MFnAnimCurve::kTangentAutoEase);
	}
	if (strcmp(type, kWordTangentAutoCustom) == 0) {
		return (MFnAnimCurve::kTangentAutoCustom);
	}
...
std::ifstream &clipFile, MAnimCurveClipboardItem &item)
//
//	Description:
//		Read a block of the std::ifstream that should contain anim curve
...
//
{
	...
	//
	MTime::Unit inputTimeUnit = MTime::kInvalid;
	...
	MTime::Unit outputTimeUnit = MTime::kInvalid;
	...
	double lLowestFrame=0;
	double lHighestFrame=0;
	advance(clipFile);
...
char *
atomReader::skipToNextParenth(char *dataType, std::ifstream &readAnim,int parenthCount)
{
...
MStatus 
atomReader::readNodes(char *dataType, std::ifstream &readAnim,	
...
MStatus 
atomReader::readAtom(std::ifstream &readAnim, atomLayerClipboard &cb,MSelectionList &mList, atomNodeNameReplacer & replacer, 
								...
								bool &removeExportEditsFile)
{
	if (!readAnim.eof() && dataType != NULL && strcmp(dataType, kExportEditsDataString) == 0)
	{
		...
		if(status == MS::kSuccess && tempFile.length())
		{
            ...
            std::ofstream editFile(tempFile.asChar());
...
char * atomReader::putCachedOnAnimCurve(MString &nodeName,MString &fullAttributeName,
	char *dataType, std::ifstream &readAnim, MPlug &plug, MFnAnimCurve &curve)
{
	MObject attribute = plug.attribute();
	char c = readAnim.peek();
	if(readAnim && c != kBraceRightChar)
...
char * 
atomReader::readHeader(std::ifstream &readAnim, bool &hasVersionString,double &startTime, double &endTime, double &startUnitless, double &endUnitless)
//
//	Description:
//	Read the header.
...
MStatus 
atomWriter::writeClipboard(	std::ofstream& animFile, 
...
void atomWriter::writeStaticValues(std::ofstream &animFile,
...
void atomWriter::writeCachedValues(std::ofstream &animFile,
	atomCachedPlugs *cachedPlugs,
	std::set<std::string> &attrStrings,
	const MString &nodeName,
	unsigned int depth,
...
void	
atomWriter::writeNodeStart(std::ofstream& animFile,atomNodeNameReplacer::NodeType nodeType, MString &nodeName, unsigned int depth, unsigned int childCount)
{
...
void	
atomWriter::writeNodeEnd(std::ofstream& animFile)
{
	animFile << "}\n";
}


bool atomWriter::writeHeader(std::ofstream& clip,bool useSpecifiedRange, MTime &defaultStartTime, MTime &defaultEndTime)
//
//	Description:
...
//
{
	...
	clip << kAtomVersion << kSpaceChar << kAtomVersionString << kSemiColonChar << std::endl;
	clip << kMayaVersion << kSpaceChar << MGlobal::mayaVersion() << kSemiColonChar << std::endl;
	...
	if(status == MS::kSuccess && sceneName.length() >0) //if no name then don't save it
		clip << kMayaSceneFile << kSpaceChar << sceneName << kSemiColonChar << std::endl;
	MString unit;
...
bool atomWriter::writeExportEditsFilePresent(std::ofstream& clip)
//
//	Description:
//		Says that an export edits file will be present embedded at the end of the file
//
{
	clip << kExportEditsString << kSpaceChar  << kSemiColonChar << std::endl;
	return true;
}

bool atomWriter::writeExportEditsFile(std::ofstream &clip, const MString& filename)
//
//	Description:
...
bool atomWriter::writeAnim(	std::ofstream &clip, 
...
bool atomWriter::writeAnimCurve(std::ofstream &clip,
...
//
{
	...
	clip << kTwoSpace<< kAnimData << kSpaceChar << kBraceLeftChar << std::endl;
	...
	clip << kFourSpace << kOutputString << kSpaceChar <<
			outputTypeAsWord(type) << kSemiColonChar << std::endl;
	...
	clip << kFourSpace << kKeysString << kSpaceChar << kBraceLeftChar << std::endl;
	...
	clip << kFourSpace << kBraceRightChar << std::endl;
...
void atomWriter::writeValue(std::ofstream & clip,MPlug &plug)
{
	MObject attribute = plug.attribute();
