...
public:
                               ...
                               const MColor&       wireColor)
    {
        ...
        if (!fActiveWireItem) {
            ...
            fActiveWireItem->setDrawMode((MGeometry::DrawMode)(MGeometry::kWireframe | MGeometry::kShaded | MGeometry::kTextured | MGeometry::kSelectionHighlighting)); 
    ...
    void toggleDormantWireItem()
    {
        if (fDormantWireItem) {
            if (fIsBoundingBoxPlaceHolder) {
                ...
                fDormantWireItem->setEnabled(fVisibility && fValidPoly);
            }
        }
    }
                                      ...
                                      const MFrameContext&      frameContext) const
{
    ...
    if (geometry && frameContext.getLightingMode() == MFrameContext::kLightDefault) {
        ...
        BoundingBoxVisitor visitor(fShapeNode->getEffectiveTime().as(MTime::kSeconds));
                              ...
                              const MFrameContext& frameContext)
{
    ...
    fTimeInSeconds = fShapeNode->getEffectiveTime().as(MTime::kSeconds);
