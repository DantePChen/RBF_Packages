	...
	std::string&	errors )	const
{
	...
	if( ((const char*)streamNode.name == NULL)
        || 0 != strncmp( (const char*)streamNode.name, xmlTagStream, sizeof(xmlTagStream) ) )
	{
		return NULL;
	}
	...
	if( streamNode.children && newStream )
	{
			 ...
			 dataNode; dataNode = dataNode->next )
		{
			...
			if( (dataNode->type != XML_ELEMENT_NODE)
                || ((const char*)dataNode->name == NULL)
			    || (0 != strncmp( (const char*)dataNode->name, xmlTagStreamData, sizeof(xmlTagStreamData) ) ) )
			{
				continue;
			}
				 ...
				 valueNode; valueNode = valueNode->next )
			{
				...
				if( ((const char*)valueNode->name != NULL)
                    && (0 == strncmp( (const char*)valueNode->name, xmlTagStreamDataIndex, sizeof(xmlTagStreamDataIndex) )) )
				{
					// Set the index value
