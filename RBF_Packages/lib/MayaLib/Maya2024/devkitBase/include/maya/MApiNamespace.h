...
#if defined(OPENMAYA_DLL) || defined(OPENMAYAUI_DLL) || defined(OPENMAYAANIM_DLL) || defined(OPENMAYAFX_DLL) || defined(OPENMAYARENDER_DLL)
#   define OPENMAYA_PRIVATE public
#else
#   define OPENMAYA_PRIVATE private
#endif

#if defined(OPENMAYA_DLL) || defined(OPENMAYAUI_DLL) || defined(OPENMAYAANIM_DLL) || defined(OPENMAYAFX_DLL) || defined(OPENMAYARENDER_DLL) || \
    defined(PYTHON_OPENMAYA_DLL) || defined(PYTHON_OPENMAYAUI_DLL) || defined(PYTHON_OPENMAYAANIM_DLL) || defined(PYTHON_OPENMAYAFX_DLL) || defined(PYTHON_OPENMAYARENDER_DLL)
#   define PYTHON_OPENMAYA_PRIVATE public
#else
#   define PYTHON_OPENMAYA_PRIVATE private
#endif

#if defined(SWIG_PYTHON)
#   define OPENMAYA_PRIVATE_NO_SWIG public
#else
#   define OPENMAYA_PRIVATE_NO_SWIG OPENMAYA_PRIVATE
#endif // OPENMAYA_PRIVATE_NO_SWIG symbols are meant to be only exposed through the python API

#ifndef OPENMAYA_EXPORT	// Could have already been defined in HAWExport.h
#	if defined( OPENMAYA_DLL )
#		define OPENMAYA_EXPORT DLL_EXPORT
#	else
#		define OPENMAYA_EXPORT DLL_IMPORT
#	endif

/* OpenMayaUI */
#	if defined( OPENMAYAUI_DLL )
#		define OPENMAYAUI_EXPORT DLL_EXPORT
#	else
#		define OPENMAYAUI_EXPORT DLL_IMPORT
#	endif

/* OpenMayaAnim */
#	if defined( OPENMAYAANIM_DLL )
#		define OPENMAYAANIM_EXPORT DLL_EXPORT
#	else
#		define OPENMAYAANIM_EXPORT DLL_IMPORT
#	endif

/* OpenMayaFX */
#	if defined( OPENMAYAFX_DLL )
#		define OPENMAYAFX_EXPORT DLL_EXPORT
#	else
#		define OPENMAYAFX_EXPORT DLL_IMPORT
#	endif

/* OpenMayaRender */
#	if defined( OPENMAYARENDER_DLL )
#		define OPENMAYARENDER_EXPORT DLL_EXPORT
#	else
#		define OPENMAYARENDER_EXPORT DLL_IMPORT
#	endif
#endif
...
#ifndef FND_EXPORT
#	if defined( FND_DLL )   
#		define FND_EXPORT DLL_EXPORT
#	else
#		define FND_EXPORT DLL_IMPORT
#	endif
#endif

//IMAGE
#ifndef IMAGE_EXPORT
#	if defined( IMAGE_DLL )
#		define IMAGE_EXPORT DLL_EXPORT
#	else
#		define IMAGE_EXPORT DLL_IMPORT
#	endif
#endif

#define BEGIN_INTERNAL_CLASS_FORWARD_DECLARE
#define END_INTERNAL_CLASS_FORWARD_DECLARE
...
/*
    API namespaces are used to safely modify API classes in ways that break Maya's ABI in
    already released versions of Maya. Any plugin compiled against a released version of
    Maya works in any future version of Maya with the same Major release version.

    To make a change which would normally break the ABI safe:
     - Define a namespace for the minor release that the change will be shipped in.
     - Derive a new version of the API class from the existing class in the new namespace.
     - Forward declare your new class here in MApiNamespace.
     - Make your ABI breaking changes in the derived class.
     - In the plugin code, the user will have to explicitly qualify their use of the newer class
       using the new namespace to use the new functionality. There is already a "using" statement
       for the class with the Major namespace, so all usage of the new class must be fully qualified.
     - If modifying MStatus or MObject, update Tplugin.cpp so that we get the name mangling for
       initializePlugin and uninitializePlugin correct.
    
    Only classes with ABI breaking changes exist in each namespace.
    
    Hypothetical Example:
    In 20230100 there is an ABI breaking change for MPxSubSceneOverride. In 20230100 there exist two
    classes:
     - Autodesk::Maya::20230000::MHWRender::MPxSubSceneOverride
     - Autodesk::Maya::20230100::MHWRender::MPxSubSceneOverride
    Plugins compiled against 20230000 work in 20230000 and 20230100.
    Plugins compiled against 20230100 work in 20230100. These plugins do not work in 20230000.
    Then, in 20230200 there is an ABI breaking change for MVertexBuffer. In 20230200 there exist 4 classes:
     - Autodesk::Maya::20230000::MHWRender::MPxSubSceneOverride
     - Autodesk::Maya::20230100::MHWRender::MPxSubSceneOverride
     - Autodesk::Maya::20230000::MHWRender::MVertexBuffer
     - Autodesk::Maya::20230200::MHWRender::MVertexBuffer
    Note that there is no 20230200 MPxSubSceneOverride, and no 20230100 MVertexBuffer.
    Plugins compiled against 20230000 work in 20230000, 20230100 and 20230200.
    Plugins compiled against 20230100 work in 20230100 and 20230200. These plugins do not work in 20230000.
    Plugins compiled against 20230200 work in 20230200. These plugins do not work in 20230000 or 20230100.
*/

// Namespace for Major release
#define OPENMAYA_MAJOR_NAMESPACE OpenMaya20240000
...
// Namespaces for Minor releases
// Example:
// #define OPENMAYA_20230100_NAMESPACE OpenMaya20230100
// #define OPENMAYA_20230100_NAMESPACE_OPEN OPENMAYA_NAMESPACE_OPEN(OPENMAYA_20230100_NAMESPACE)
// #define OPENMAYA_20230100_NAMESPACE_SCHEMA OPENMAYA_NAMESPACE_SCHEMA(OPENMAYA_20230100_NAMESPACE)
...
#define OPENMAYA_LATEST_NAMESPACE
#define OPENMAYA_LATEST_NAMESPACE_OPEN
#define OPENMAYA_LATEST_NAMESPACE_SCHEMA

#endif /* SWIG */

#if defined(OPENMAYA_STRICT_NO_OVERRIDE)
#define OPENMAYA_NO_OVERRIDE final
#else
#define OPENMAYA_NO_OVERRIDE
#endif
...
#define USING_MAJOR_ONLY_API \
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MFnDisplayLayer;                    \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MFnDisplayLayerManager;             \
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MPxGPUStandardDeformer;             \
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MCacheSchema;                       \
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MDisplayLayerMessage;               \
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MFnFloatVectorArrayData;            \
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MFnStandardSurfaceShader;           \
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MFnUintArrayData;                   \
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MNodeCacheDisablingInfo;            \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MNodeCacheDisablingInfoHelper;      \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MNodeCacheSetupInfo;                \
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MAutoCLEventList;                   \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MOpenCLBuffer;                      \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MOpenCLInfo;                        \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MOpenCLKernelInfo;                  \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MOpenCLUtils;                       \
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MSharedPtr;                         \
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MTimeRange;                         \
	...
	using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MDrawProperties;				\
	using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MPanelCanvas;					\
	using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MPanelCanvasInfo;					\
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MColorMixingSpaceHelper;                     \
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MIndexMapper;	        					\
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MFnFalloffData;	        					\
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MSyncId;	        					\
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MFalloffContext;	                    \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MFalloffFunction;        					\
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MTimeSliderCustomDrawManager;                \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MTimeSliderDrawPrimitives;                             \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MTimeSliderDrawPrimitive;                              \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MStartPrimitiveEditingFct;    \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MEditPrimitiveFct;            \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MSetCopyPrimitivesFct;            \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MStopPrimitiveEditingFct;     \
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MBufferIdentifier;        					\
    ...
    using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MHWRender::MPxViewportComputeItem;           \
	...
	using OPENMAYA_MAJOR_NAMESPACE_SCHEMA::MHWRender::MDataServerOperation;			\
