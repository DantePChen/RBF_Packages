...
# while the methods:
#   footPrintGeometryOverrideAnimatedMaterial.updateDG() 
#   footPrintGeometryOverrideAnimatedMaterial.updateRenderItems() 
#   footPrintGeometryOverrideAnimatedMaterial.populateGeometry()
...
# cmds.loadPlugin('footPrintNode_GeometryOverride_AnimatedMaterial')
# cmds.createNode('footPrint_GeometryOverride_AnimatedMaterial')
#
#########################################################################
import sys
import ctypes
...
def get3dSolidShader(color):
	...
	if shader:
		shader.setParameter(footPrintGeometryOverrideAnimatedMaterial.colorParameterName_, color)
...
def releaseShaders():
	shaderMgr = omr.MRenderer.getShaderManager()
	if shaderMgr:
		for key, shader in the3dSolidShaders.items():
...
class footPrint(omui.MPxLocatorNode):
	id = om.MTypeId( 0x00080033 )
	drawDbClassification = "drawdb/geometry/footPrint_GeometryOverride_AnimatedMaterial"
	drawRegistrantId = "footPrintNode_GeometryOverride_AnimatedMaterialPlugin"
	...
	def getShapeSelectionMask(self):
		selType = om.MSelectionMask("footPrintSelection")
...
class footPrintGeometryOverrideAnimatedMaterial(omr.MPxGeometryOverride):
	...
	@staticmethod
	def creator(obj):
		return footPrintGeometryOverrideAnimatedMaterial(obj)
	...
	def updateRenderItems(self, dagPath, renderList ):
		...
		fullItemList = ( 
			(footPrintGeometryOverrideAnimatedMaterial.wireframeItemName_, omr.MGeometry.kLines, omr.MGeometry.kWireframe),
			(footPrintGeometryOverrideAnimatedMaterial.shadedItemName_, omr.MGeometry.kTriangles, omr.MGeometry.kShaded)
		)
	...
	def populateGeometry(self, requirements, renderItems, data):
		...
		for item in renderItems:
			...
			if item.name() == footPrintGeometryOverrideAnimatedMaterial.wireframeItemName_:
			...
			elif item.name() == footPrintGeometryOverrideAnimatedMaterial.shadedItemName_:
				primitiveIndex = 0
				startIndex = 0
...
def initializePlugin(obj):
	...
	try:
		plugin.registerNode("footPrint_GeometryOverride", footPrint.id, footPrint.creator, footPrint.initialize, om.MPxNode.kLocatorNode, footPrint.drawDbClassification)
	except:
		sys.stderr.write("Failed to register node\n")
		raise

	try:
		omr.MDrawRegistry.registerGeometryOverrideCreator(footPrint.drawDbClassification, footPrint.drawRegistrantId, footPrintGeometryOverrideAnimatedMaterial.creator)
	except:
		sys.stderr.write("Failed to register override\n")
		raise

	try:
		om.MSelectionMask.registerSelectionType("footPrintSelection")
		mel.eval("selectType -byName \"footPrintSelection\" 1")
	except:
...
def uninitializePlugin(obj):
	...
	try:
		om.MSelectionMask.deregisterSelectionType("footPrintSelection") 
	except:
