    ...
    const MDagPath& cameraPath) const
{
    ...
    const double seconds = effectiveTime.as(MTime::kSeconds);
    ...
    MUserData* oldData)
{
    ...
    if (data) {
		...
		// If possible, get the shape's effective time, otherwise, fall back to
		// the currentTime.
		//
		const ShapeNode* shapeNode = status?dynamic_cast<ShapeNode*>(node.userNode()):nullptr;
		MTime effectiveTime = shapeNode?(shapeNode->getEffectiveTime()):(MAnimControl::currentTime());

        data->set(
            effectiveTime.as(MTime::kSeconds),
            wireframeColor,
            isSelected);
    }
