...
// for the purposes of showing how point light shadow maps(cube map) may be requested "on-demand" 
// and used for selective lighting (and shadowing) in a scene render.
//
// The basic logic is:
//
// 1. Perform an operation to queue requests for shadow map computation for point lights. The
...
// 3. preSceneRender() is a method to allow for the operation to update itself
...
// 4. setup() perform any setup required before render operations are to be executed.
//		The default behaviour performs no actions. An override is called once per render.
//		The order of method calls when an override is called is as follows:
//		- setup()
//			- startOperationIterator()
//			- renderOperation()
//			- nextRenderOperation() until no more operations
//		- cleanup()
//
///////////////////////////////////////////////////////////////////////////////
/*
...
*/
viewRenderOverridePointLightShadows::viewRenderOverridePointLightShadows(const MString& name)
: MRenderOverride(name)
, mUIName("Sample VP2 Point Light Shadow Requester")
{
	unsigned int i = 0;
...
*/
viewRenderOverridePointLightShadows::~viewRenderOverridePointLightShadows()
{
...
*/
MHWRender::DrawAPI viewRenderOverridePointLightShadows::supportedDrawAPIs() const
{
...
//
bool viewRenderOverridePointLightShadows::startOperationIterator()
{
	mCurrentOperation = 0;
	return true;
}

MHWRender::MRenderOperation* viewRenderOverridePointLightShadows::renderOperation()
{
...
bool viewRenderOverridePointLightShadows::nextRenderOperation()
{
	mCurrentOperation++;
	if (mCurrentOperation < kOperationCount)
	{
...
//
MStatus viewRenderOverridePointLightShadows::updateRenderOperations()
{
	...
	if (initOperations)
	{
		mRenderOperationNames[kShadowPrePass] = "_viewRenderOverridePointLightShadows_ShadowPrepass";
		...
		mRenderOperationNames[kMaya3dSceneRender] = "_viewRenderOverridePointLightShadows_SceneRender";
		...
		mRenderOperationNames[kPresentOp] = "_viewRenderOverridePointLightShadows_PresentTarget";
...
//
MStatus viewRenderOverridePointLightShadows::updateLightList()
{
	mLightList.clear();
	...
	for (unsigned int i=0; i<selectList.length(); i++)
	{
		...
		// Get dependNode classification.
		MString drawdb = dependNode.classification(dependNode.typeName());

		// Compare with point light classification to check if this node present a point light.
		MString pointLightClassification = MString("drawdb/light/pointLight");
		int result = memcmp(drawdb.asWChar(), pointLightClassification.asWChar(), pointLightClassification.numChars()*sizeof(wchar_t));

		// Prune by pointLight classification.
		if ( result==0 )
		{
						...
						const MSelectionList * lightList )
{
	...
	for (unsigned int i=0; i<passSem.length() && handlePass; i++)
	{
		...
		//
		if ((passSem[i] == MHWRender::MPassContext::kShadowPassSemantic)||(passSem[i] == MHWRender::MPassContext::kPointLightShadowPassSemantic))
		{
			handlePass = false;
		}
	}
	...
	MFloatVector lightPosition(0.0f, 0.0f, 0.0f);
	float lightIntensity = 0.0f;		// If no lights then black out the light
	float lightColor[3] = { 0.0f, 0.0f, 0.0f };
	float range[2] = { 0.0f, 0.0f };	// This indicate far and near clip of point light camera.
	float bias = 0.0f;
	MStatus status;
	...
	if (lightCount)
	{
		...
		for (i=0; i<lightCount; i++)
		{
			...
			if (lightParam)
			{
				...
				for (unsigned int p=0; p<params.length(); p++)
				{
					...
					case MHWRender::MLightParameterInformation::kWorldPosition:
						...
						lightPosition = MFloatVector( floatVals[0], floatVals[1], floatVals[2] );
						break;
					...
					case MHWRender::MLightParameterInformation::kShadowColor:
						...
						break;
					case MHWRender::MLightParameterInformation::kDepthRange:
						lightParam->getParameter( pname, floatVals );
						range[0] = floatVals[0];
						range[1] = floatVals[1];
						break;
			...
			if (globalShadowsOn && localShadowsOn && shadowResource.texture)
			{
				void *resourceHandle = shadowResource.texture->resourceHandle();
				if (resourceHandle)
				{
					static bool debugShadowBindings = false;
					status  = shaderInstance->setParameter("mayaShadowPointLight_shadowMap", shadowResource );
					if (status == MStatus::kSuccess && debugShadowBindings)
						fprintf(stderr, "Bound shadow map to shader param mayaShadowPointLight_shadowMap\n");
					status  = shaderInstance->setParameter("mayaShadowPointLight_shadowColor", &shadowColor[0] );
					if (status == MStatus::kSuccess && debugShadowBindings)
						fprintf(stderr, "Bound shadow map color to shader param mayaShadowPointLight_shadowColor\n");
				}
				...
				if (renderer)
				{
					...
					if (textureManager)
					{
						// In MLightParameterInformation::getParameter(), we have create a texture pointer. When leaving the scope, it need to be released.
	...
	status = shaderInstance->setParameter("mayaPointLight_intensity", lightIntensity );
	status = shaderInstance->setParameter("mayaPointLight_color", &( lightColor[0] ));
	status = shaderInstance->setParameter("mayaShadowPointLight_depthRange", &( range[0] ));	
	status = shaderInstance->setParameter("mayaShadowPointLight_shadowBias", bias );
	status = shaderInstance->setParameter("mayaShadowPointLight_mayaGlobalShadowOn", globalShadowsOn);
	status = shaderInstance->setParameter("mayaShadowPointLight_mayaShadowOn", localShadowsOn);
}

//
// Update the light shader override used for rendering the scene with.
//
MStatus viewRenderOverridePointLightShadows::updateShaders(const MHWRender::MShaderManager* shaderMgr)
{
	...
	//
	const MString shaderName("MayaBlinnPointLightShadow");
...
//
MStatus viewRenderOverridePointLightShadows::setup(const MString& destination)
{
	// Firewall checks
	MHWRender::MRenderer* renderer = MHWRender::MRenderer::theRenderer();
...
*/
MStatus viewRenderOverridePointLightShadows::cleanup()
{
	mCurrentOperation = -1;
