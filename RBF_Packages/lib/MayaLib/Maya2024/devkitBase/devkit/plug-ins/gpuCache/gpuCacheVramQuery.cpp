...
namespace {
    ...
    std::string getXorgLogFilePath()
    {
        // Provide a workaround to specify the Xorg log path in the case of a weird
        // installation.
        const char* xorgLogFilePathOverride = ::getenv("MAYA_XORG_LOG_FILE_PATH_OVERRIDE");
        if (xorgLogFilePathOverride)
            return xorgLogFilePathOverride;

        // Default string on most systems.
        const char* kDefaultFilePath = "/var/log/Xorg.0.log";

        // Get the current display in case the X server was started on a display number
        // different than 0.
        char* host = nullptr;
        int display = 0;
        const int xcbSuccess = xcb_parse_display(nullptr, &host, &display, nullptr);
        if (!xcbSuccess)
            return kDefaultFilePath;

        // We don't care about the returned host.  However, the approach of parsing
        // the local Xorg log file to determine access to graphics hardware will not
        // work if the X server is on a remote host.  However, I'm guessing a lot of
        // other stuff will be broken in that case as well, and the host could still
        // be referring to something like "localhost" or "127.0.0.1" and we don't
        // want to handle these possibilities.
        ::free(host);

        // In a similar way, the default location for the Xorg log can be set in
        // many ways:
        // - Default path at server compilation time
        // - Configuration file
        // - Command line argument
        // - Etc.
        // We don't really have a way to query for it, so we assume the default;
        // we only adjust for display number.
        std::ostringstream o;
        o << "/var/log/Xorg." << display << ".log";
        return o.str();
    }
...
size_t queryVramMAC()
{
    ...
    // Get all the number of active displays (multi-monitor support)
    CGError cgError = CGGetActiveDisplayList(0, NULL, &nDisplayCount);
    if(cgError == kCGErrorSuccess && nDisplayCount > 0)
    {
        CGDirectDisplayID* pDisplayIDs = new CGDirectDisplayID[nDisplayCount];
        if (pDisplayIDs != NULL)
        {
            // Populate an array with active display Ids
            cgError = CGGetActiveDisplayList(nDisplayCount, pDisplayIDs, &nDisplayCount);
            if (cgError == kCGErrorSuccess)
            {
                GLint nCount = 0;
                CGLRendererInfoObj pInfo = NULL;
                CGOpenGLDisplayMask nMask = 0;
                for (GLuint i=0; i<nDisplayCount; ++i)
                {
                    // For every active display get the OpenGL display mask
                    nMask = CGDisplayIDToOpenGLDisplayMask(pDisplayIDs[i]);

                    // For every OpenGL display mask get their renderer info object
                    CGLError cglError = CGLQueryRendererInfo(nMask, &pInfo, &nCount);
                    if (cglError == kCGLNoError && nullptr != pInfo)
                    {
                        // For each renderer info object get the actual number of renderers
                        cglError = CGLDescribeRenderer(pInfo, 0, kCGLRPRendererCount, &nCount);
                        if (cglError == kCGLNoError)
                        {
                            for (GLuint j=0; j<nCount; ++j)
                            {
                                // For each renderer get the renderer id
                                GLint vram = 0;
                                cglError = CGLDescribeRenderer(pInfo, j, kCGLRPVideoMemoryMegabytes, &vram);
                                if (cglError == kCGLNoError)
                                {
                                    if (vram > maxVram)
                                    {
    ...
    // vram size is in MBs, expected return in bytes
    return maxVram * 1024 * 1024;
}
...
void VramQuery::queryVramAndDriverMAC(MUint64& vram, int driverVersion[3], MString& manufacturer, MString& model)
{
    ...
    for (CGDisplayCount i = 0; i < dspCount; i++) {
    ...
    vram = (MUint64)queryVramMAC();

    if (manufacturer == "") {
       manufacturer = MString((const char *)glGetString(GL_RENDERER));
    }
...
void VramQuery::queryVramAndDriverXORG(MUint64& vram, int driverVersion[3], MString& manufacturer, MString& model)
{
    ...
    // open the Xorg.0.log
    std::string   line;
    std::ifstream xorgLog(getXorgLogFilePath());
    if (!xorgLog.is_open()) {
        return;
    }
