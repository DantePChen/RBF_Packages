...
namespace apiMeshSubSceneOverrideHelpers
{
	...
	class LinkLostUserData : public MUserData
	{
	public:
		LinkLostUserData(apiMeshSubSceneOverride* ov)
		    : MUserData()
            , fOverride(ov)
        {}

		apiMeshSubSceneOverride* fOverride = nullptr;
	};
	...
	public:
		...
		apiMeshGeom* fMeshGeom = nullptr;
        int fInstanceIndex = 0;
		std::unique_ptr<bool[]> fFaceViewSelectedStates;
	};

	using LinkLostUserDataPtr = MSharedPtr<LinkLostUserData>;
	using ApiMeshHWSelectionUserDataPtr = MSharedPtr<apiMeshHWSelectionUserData>;
	...
	public:
		...
		void initialize(const MHWRender::MRenderItem& renderItem) override
		{
			...
			if (fComponentType == MFn::kMeshPolygonComponent) {
				ApiMeshHWSelectionUserDataPtr selectionData = ApiMeshHWSelectionUserDataPtr::template dynamic_pointer_cast(renderItem.getCustomData());
				if(selectionData) {
...
const MString apiMeshSubSceneOverride::sWireName("apiMeshWire_subscene");
const MString apiMeshSubSceneOverride::sSelectName("apiMeshSelection_subscene");
const MString apiMeshSubSceneOverride::sBoxName("apiMeshBox_subscene");
const MString apiMeshSubSceneOverride::sSelectedBoxName("apiMeshBoxSelection_subscene");
const MString apiMeshSubSceneOverride::sShadedName("apiMeshShaded_subscene");
const MString apiMeshSubSceneOverride::sTexturedName("apiMeshTextured_subscene");

const MString apiMeshSubSceneOverride::sVertexSelectionName("apiMeshVertexSelection_subscene");
const MString apiMeshSubSceneOverride::sEdgeSelectionName("apiMeshEdgeSelection_subscene");
const MString apiMeshSubSceneOverride::sFaceSelectionName("apiMeshFaceSelection_subscene");

const MString apiMeshSubSceneOverride::sActiveVertexName("apiMeshActiveVertex_subscene");
const MString apiMeshSubSceneOverride::sActiveEdgeName("apiMeshActiveEdge_subscene");
const MString apiMeshSubSceneOverride::sActiveFaceName("apiMeshActiveFace_subscene");
...
apiMeshSubSceneOverride::~apiMeshSubSceneOverride()
{
	...
	for (
		auto userData = fLinkLostUserDatas.begin(); 
		userData != fLinkLostUserDatas.end(); 
		userData++)
    {
        (*userData)->fOverride = nullptr;
    }
...
void apiMeshSubSceneOverride::untrackLinkLostData(LinkLostUserData* data)
{
	fLinkLostUserDatas.erase(
		std::remove_if(
			fLinkLostUserDatas.begin(),
			fLinkLostUserDatas.end(),
			[&data](const LinkLostUserDataPtr& arg) { return arg.get() == data; }),
		fLinkLostUserDatas.end()
	);
}
...
void apiMeshSubSceneOverride::updateRenderItemShader(
...
)
{
    if (!surfaceShaderNode.isNull()
        && (updateMaterial || !renderItem.isShaderFromNode())
    )
    {
		auto userData = LinkLostUserDataPtr(new LinkLostUserData(this));
		if (renderItem.setShaderFromNode2(
			surfaceShaderNode,
			dagPath,
			onShadedItemLinkLost,
			userData,
			nonTextured
		))
        {
			auto setcustom = MSharedPtr<MUserData>::make(MUserData());
			renderItem.setCustomData(setcustom);
			auto custom = renderItem.getCustomData();
			std::cout << "" << std::endl;

            fLinkLostUserDatas.push_back(userData);
            return;
        }
    ...
    if (updateMaterial)
    {
        renderItem.setShader(fShadedShader);
    }
}
	...
	bool updateGeometry)
{
		...
		for (unsigned int i = 0; i < sets.length(); i++)
		{
			...
			if (status)
			{
				...
				if (!shaderPlug.isNull())
				{
					...
					{
						surfaceShaderNode = connectedPlugs[0].node(&status);
						if (status && !surfaceShaderNode.isNull())
                            break;
					}
				}
			}
    ...
    updateRenderItemShader(*shadedItem, updateMaterial, surfaceShaderNode, instances[0], true);
    updateRenderItemShader(*texturedItem, updateMaterial, surfaceShaderNode, instances[0], false);
	...
	{
		auto mySelectionData = ApiMeshHWSelectionUserDataPtr(new apiMeshHWSelectionUserData());
	...
	if (itemsChanged || updateGeometry)
	{
		...
		wireBuffers.addBuffer("positions", fPositionBuffer.get());
		...
		boxBuffers.addBuffer("positions", fBoxPositionBuffer.get());
		...
		shadedBuffers.addBuffer("positions", fPositionBuffer.get());
		shadedBuffers.addBuffer("normals", fNormalBuffer.get());
	...
	if (itemsChanged || updateActiveItems)
	{
		...
		vertexBuffer.addBuffer("positions", fPositionBuffer.get());
	...
	manageIsolateSelectRenderItems(container, frameContext, instances, vsfInfo,
		surfaceShaderNode, updateMaterial, updateGeometry);
	...
	const bool updateGeometry)
{
	...
	if (updateViewSelectedFaces)
	{
		...
		for (int instIdx : currInstIdxArray)
		{
			...
			if (!viewSelectedShadedItem)
			{
				...
				auto userData = ApiMeshHWSelectionUserDataPtr(new apiMeshHWSelectionUserData());
	...
	for (auto& p : fViewSelectedFaceInfoCache)
	{
		...
		for (int instIdx : instIdxArray)
		{
			...
			{
                updateRenderItemShader(*viewSelectedShadedItem, updateMaterial, surfaceShaderNode, instance, true);
                updateRenderItemShader(*viewSelectedTexturedItem, updateMaterial, surfaceShaderNode, instance, false);
				...
				selectionBuffers.addBuffer("positions", fPositionBuffer.get());
				...
				ApiMeshHWSelectionUserDataPtr userData;
				userData = ApiMeshHWSelectionUserDataPtr::template dynamic_pointer_cast(viewSelectedShadedItem->getCustomData());				
				...
				userData = ApiMeshHWSelectionUserDataPtr::template dynamic_pointer_cast(viewSelectedTexturedItem->getCustomData());
				if (userData)
				{
					userData->fMeshGeom = meshGeom;
				}

				userData = ApiMeshHWSelectionUserDataPtr::template dynamic_pointer_cast(viewSelectedFaceSelectionItem->getCustomData());
				if (userData)
				{
					userData->fMeshGeom = meshGeom;
...
void apiMeshSubSceneOverride::rebuildGeometryBuffers()
{
	...
	fPositionBuffer = std::make_unique<MVertexBuffer>(posDesc);
	fNormalBuffer = std::make_unique<MVertexBuffer>(normalDesc);
	fBoxPositionBuffer = std::make_unique<MVertexBuffer>(posDesc);
	...
	fWireIndexBuffer = std::make_unique<MIndexBuffer>(MGeometry::kUnsignedInt32);
	fBoxIndexBuffer = std::make_unique<MIndexBuffer>(MGeometry::kUnsignedInt16);
	fShadedIndexBuffer = std::make_unique<MIndexBuffer>(MGeometry::kUnsignedInt32);
		...
		{		
			{
				...
				{
					glBindBufferARB(GL_ARRAY_BUFFER_ARB, fBoxPositionBufferId);
					glBufferDataARB(GL_ARRAY_BUFFER_ARB, 8*3*sizeof(float), boxPositions, GL_STATIC_DRAW_ARB);
					glBindBufferARB(GL_ARRAY_BUFFER_ARB, 0);
					fBoxPositionBuffer->resourceHandle(&fBoxPositionBufferId, 8*3);
				}

				glGenBuffersARB(1, &fBoxIndexBufferId);
				if (fBoxIndexBufferId != 0)
				{
					glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, fBoxIndexBufferId);
					glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, 24*sizeof(unsigned short), boxIndices, GL_STATIC_DRAW_ARB);
...
void apiMeshSubSceneOverride::rebuildActiveComponentIndexBuffers()
{
	...
	{
		fActiveVerticesIndexBuffer = std::make_unique<MIndexBuffer>(MGeometry::kUnsignedInt32);
	...
	if (numActiveEdges > 0)
	{
		fActiveEdgesIndexBuffer = std::make_unique<MIndexBuffer>(MGeometry::kUnsignedInt32);
	...
	if (numActiveFaces > 0)
	{
		...
		fActiveFacesIndexBuffer = std::make_unique<MIndexBuffer>(MGeometry::kUnsignedInt32);
...
{
	fPositionBuffer.reset();
	fNormalBuffer.reset();
	fBoxPositionBuffer.reset();
	fWireIndexBuffer.reset();
	fBoxIndexBuffer.reset();
	fShadedIndexBuffer.reset();
	...
	if (sUseCustomUserBuffersForBoundingBox)
	{
		...
		{					
			...
			if (fBoxIndexBufferId != 0)
			{
				glDeleteBuffersARB(1, &fBoxIndexBufferId);
				fBoxIndexBufferId = 0;
			}
		}
#ifdef _WINDOWS
...
{
	fActiveVerticesIndexBuffer.reset();
	fActiveEdgesIndexBuffer.reset();
	fActiveFacesIndexBuffer.reset();
}
    ...
    MDagPath& dagPath) const
{
    ...
    if (instanceCount == 1 || instanceId == -1)
    {
        ...
        ApiMeshHWSelectionUserDataPtr userData = ApiMeshHWSelectionUserDataPtr:: template dynamic_pointer_cast(renderItem.getCustomData());
        if (userData &&
...
/*static*/
void apiMeshSubSceneOverride::onShadedItemLinkLost(MUserData* userData)
{
	auto* data = dynamic_cast<LinkLostUserData*>(userData);
