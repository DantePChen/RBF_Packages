...
class OPENMAYA_DEPRECATED(2020, "Use std::atomic") MAtomic
{
public:
...
#else
    return __atomic_fetch_add(variable, incrementValue, __ATOMIC_SEQ_CST);
#endif
}
...
#else 
    return __atomic_fetch_sub( variable, decrementValue, __ATOMIC_SEQ_CST );
#endif
}
...
#else
    return __atomic_exchange_n( variable, newValue, __ATOMIC_SEQ_CST );
#endif
}
							 ...
							 int swapValue)
{
    ...
    return (_InterlockedCompareExchange((long*)variable, swapValue, compareValue)
			== compareValue);
#else
    return __atomic_compare_exchange_n( variable, &compareValue, swapValue, 0, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST );
#endif
}
