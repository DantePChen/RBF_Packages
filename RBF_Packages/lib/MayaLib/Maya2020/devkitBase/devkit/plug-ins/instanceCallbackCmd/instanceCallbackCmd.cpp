	...
	for(iter.reset(); !iter.isDone() ; iter.next())
	{
		...
		if(item.hasFn(MFn::kNurbsCurve))
		{
			...
			if(dagPath.isValid())
			{
				...
				//
				MDagMessage::addDagCallback( dagPath, MDagMessage::DagMessage::kInstanceAdded, addCallbackFunc,	NULL, &status);
				MDagMessage::addDagCallback( dagPath, MDagMessage::DagMessage::kInstanceRemoved, remCallbackFunc, NULL, &status);
