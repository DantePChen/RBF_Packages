...
#
# This plug-in implements a proxy surface shape node that will display rectangles,
# triangles, and circles using standard OpenGL calls.
# 
# There are no output attributes for this shape. The following input attributes
# define the type of shape to draw:
#
...
#
# To create one of these nodes, enter the following commands after the plug-in is loaded:
#
#	import maya.cmds as cmds
#	cmds.createNode("spBasicShape")
# 
# An object will be created with reference to the node.  By default, it will be a rectangle.
# Use the different node options to change the type of shape or its size and shape.
...
spBasicShapeNodeId = OpenMaya.MTypeId(0x0008005e)
...
class basicShapeUI(OpenMayaMPx.MPxSurfaceShapeUI):
	# private enums
	__kDrawRectangle, __kDrawCircle, __kDrawTriangle = list(range(3))
	__kDrawWireframe, __kDrawWireframeOnShaded, __kDrawSmoothShaded, __kDrawFlatShaded, __kLastToken = list(range(5))
