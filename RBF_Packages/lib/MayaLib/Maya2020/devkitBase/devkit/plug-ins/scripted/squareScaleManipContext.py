...
########################################################################
#
# First make sure that squareScaleManipContext.py is in your 
# MAYA_PLUG_IN_PATH.  Then, to use the tool, execute the following in the 
# script editor:
#
# from maya import cmds
# cmds.loadPlugin("squareScaleManipContext.py")
# ctx = cmds.squareScaleManipContext()
# cmds.setToolTo(ctx)
#
# Once the tool is active, click on an object. Move the right and left 
...
class SquareScaleManipulator (OpenMayaUI.MPxManipulatorNode):
    ...
    """
    kNodeName = 'SquareScaleContextManipulator'
    kTypeId = OpenMaya.MTypeId( 0x81048 )
...
class SquareScaleManipContext(OpenMayaUI.MPxSelectionContext):
    ...
    def update_manipulators_cb(ctx):
        ...
        try:
            ...
            (manipulator, manip_object) = SquareScaleManipulator.newManipulator('SquareScaleContextManipulator')
