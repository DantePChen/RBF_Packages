...
const MString ShapeNode::sLongNameCacheStartFrame = "cacheStartFrame";
const MString ShapeNode::sShortNameCacheStartFrame = "csf";
        ...
        if (exitStatus == MStatus::kSuccess) {
            ...
            for (int i=0; i<sNb3dViewPostRenderCallbacks; ++i) {
                ...
                callbackId = MUiMessage::addUiDeletedCallback(
                    editorPanels[i], uiDeleted, reinterpret_cast<void*>(uintptr_t(i)), &status);
...
void ShapeNode::postConstructor()
{
	...
	// Add a dynamic attribute for the cacheStartFrame
	//
	MDGModifier modifier;

	MFnUnitAttribute unitAttrFn;
    MObject newAttr = unitAttrFn.create( sLongNameCacheStartFrame
									   , sShortNameCacheStartFrame
									   , MFnUnitAttribute::kTime, 0.0);
    unitAttrFn.setChannelBox(true);
    unitAttrFn.setInternal(true);
	MStatus stat;
	stat = modifier.addAttribute( thisMObject(), newAttr );
	if( stat == MS::kSuccess ){
		modifier.doIt();
	}
}
...
bool ShapeNode::getEdgeSnapPoint(const MPoint &rayPointSrc, const MVector &rayDirectionSrc, MPoint &theClosestPoint) {
	const double seconds = getEffectiveTime().as(MTime::kSeconds);
...
bool ShapeNode::getInternalValue( const MPlug& plug, MDataHandle& handle)
{
    ...
    else if( (plug.partialName() == sLongNameCacheStartFrame) ||
			 (plug.partialName() == sShortNameCacheStartFrame ) ){
        handle.setMTime(fCacheStartFrame);
        return true;
    }
...
)
{
	...
	const MTime oldStartFrame = fCacheStartFrame;
	...
	bool startFrameChanged = (newStartFrame != oldStartFrame );
    if (!(fileChanged || pathChanged || startFrameChanged) )
	{
        return true;
	}

	if( startFrameChanged )
	{
		// The start frame changed. Either a file with a ShapeNode was read in
		// and the cacheStartFrame attribute was set to the correct initial
		// value, or the user changed the value of cacheStartFrame. Either
		// way, it now has the correct value, and mark it as initialized so we
		// will leave it alone the next time we call refreshTimeOffset().
		//
		fCacheStartFrame = newStartFrame;
		fCacheStartFrameInitialized = true;

		// Trigger a refresh next time we need the value of fTimeOffset.
		//
		fTimeOffsetInvalid = true;

		// Trigger a refresh, so the geometry will update, in the case that the
		// user changed the value of cacheStartFrame
		//
		MGlobal::executeCommand(MString("currentTime -e ") + MAnimControl::currentTime().as(MTime::uiUnit()));
	}
	if( pathChanged )
	{
		// The geomPath changed
...
bool ShapeNode::setInternalValue( const MPlug & plug, const MDataHandle & handle)
{
    if (plug == aCacheFileName) {
        ...
        return setInternalValues(newFileName, fCacheGeomPath, fCacheStartFrame);
    }
    else if (plug == aCacheGeomPath) {
        MString newGeomPath = handle.asString();
        return setInternalValues(fCacheFileName, newGeomPath, fCacheStartFrame);
	}
    else if( (plug.partialName() == sLongNameCacheStartFrame) ||
			 (plug.partialName() == sShortNameCacheStartFrame ) ){
        MTime newStartFrame = handle.asTime();
        return setInternalValues(fCacheFileName, fCacheGeomPath, newStartFrame);
    }
...
void ShapeNode::refreshCachedGeometry( bool clearFileCache /* = false */ )
{
    ...
    const MTime cacheStartFrame = fCacheStartFrame;
    ...
    setInternalValues(cacheFileName, cacheGeomPath, cacheStartFrame);
...
CacheFileEntry::BackgroundReadingState ShapeNode::backgroundReadingState() const
{
	if( fCacheFileEntry )
	{
...
void ShapeNode::copyInternalData(MPxNode* source)
{
    if (source && source->typeId() == id) {
        ...
        fCacheStartFrame = node->fCacheStartFrame;
		fCacheStartFrameInitialized = true;
		...
		// Trigger a refresh the next time we need to access fTimeOffset
		//
		fTimeOffsetInvalid = true;
    }
}
...
// Refresh the time offset, which will initialize the start frame of the
// animation, as well as the start frame at which the user wants the
// animation to start, if need be, and re-compute the offset if necessary.
//
void ShapeNode::refreshTimeOffset()
{
	// The animation's start frame is not initialized. If the info is
	// available, initialize it now.
	//
	auto subNode = getCachedGeometry();
	if (subNode) {
		auto data = subNode->getData();
		if( data ){
			MTime animStartFrame = MTime(data->animTimeRange().startTime(), MTime::kSeconds).as(MTime::uiUnit());
			// We have a valid animStartFrame, so we can calculate a new offset
			//
			if( !fCacheStartFrameInitialized ){
				// If the cacheStartFrame hasn't been initialized (e.g., when
				// importing a file), initialize it to match animStartFrame,
				// and mark it as initialized. For example, if the .abc files
				// starts animating at frame 5, then we should initialize
				// fCacheStartFrame to 5 as well.
				//
				// If a file containing a ShapeNode has been read in,
				// cacheStartFrame will have already been initialized, and
				// marked as such, so we will skip this step.
				//
				fCacheStartFrame = animStartFrame;
				fCacheStartFrameInitialized = true;
			}
			MTime newTimeOffset( animStartFrame.as(MTime::uiUnit()) - fCacheStartFrame.as(MTime::uiUnit()) );
			if( newTimeOffset != fTimeOffset ){
				fTimeOffset = newTimeOffset;
			}
			// The value of fTimeOffset is no longer invalid; mark it as such
			//
			fTimeOffsetInvalid = false;
		}
	}
}
...
void ShapeNode::timeChangeCB(double timeInSeconds)
{
    ...
    {
        // gpuCache animates under its own power instead of depending on the DAG.  Tell
        // the Maya viewport that the node is changing so that we receive an opportunity
        // to update even when starting offscreen.
        MHWRender::MRenderer::setGeometryDrawDirty(thisMObject());
...
void ShapeUI::drawWireframe(const MDrawRequest & request, M3dView & view) const
{
	...
	const double seconds = node->getEffectiveTime().as(MTime::kSeconds);
