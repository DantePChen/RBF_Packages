...
struct AlembicCacheWriter::MakeSharedEnabler: public AlembicCacheWriter{
    MakeSharedEnabler(const MFileObject& file, char compressLevel, const MString& dataFormat)
            :AlembicCacheWriter(file, compressLevel, dataFormat){}
};
...
{
    return std::make_shared<MakeSharedEnabler>(file, compressLevel, dataFormat);
}
