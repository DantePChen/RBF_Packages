...
#if defined(__cplusplus)
// C++ 17 forbids string-litrial to mutable 'char *' conversion
// Define these inline overloads in C++ scope
// As extern "C" scope do not support overload and name mangleling 
template <typename... Args>
inline int	CapError (int client_fd, CapSeverity sev, const char *pgm, const char *fmt, Args&&... args)
{ return CapError(client_fd, sev, const_cast<char*>(pgm), const_cast<char*>(fmt), std::forward<Args>(args)...); }
inline int	CapServe (const char *server) { return CapServe(const_cast<char*>(server)); }
inline int	CapCreateInetSocket (const char *service, short def_port) { return CapCreateInetSocket(const_cast<char*>(service), def_port); }
inline int	CapCreateUnixSocket (const char *name) { return CapCreateUnixSocket(const_cast<char*>(name)); }
inline void	CapUnlinkUnixSocket (const char *name) { return CapUnlinkUnixSocket(const_cast<char*>(name)); }
inline int	CapGetAuthInfo (int client_fd, const char *ruser, const char *rhost, const char *realrhost) { return CapGetAuthInfo(client_fd, const_cast<char*>(ruser), const_cast<char*>(rhost), const_cast<char*>(realrhost)); }
inline int	CapInitialize (int client_fd, const char *name) { return CapInitialize(client_fd, const_cast<char*>(name)); }
inline int	CapVersion (int client_fd, const char *server_name, const char *server_version, const char *description) { return CapVersion(client_fd, const_cast<char*>(server_name), const_cast<char*>(server_version), const_cast<char*>(description)); }
inline CapChannel CapCreateChannel (const char *name, CapChannelUsage usage, int data_type) { return CapCreateChannel(const_cast<char*>(name), usage, data_type); }
#endif
