...
public:
    ...
    OPENMAYA_DEPRECATED(2019, "This method is no longer required in Maya3.0 and later versions. (In previous versions, this method was needed to clean up the geometry after setSmoothing has been done on one or more edges).")
	MStatus		cleanupSmoothing();
    int         index( int index, MStatus * ReturnStatus = NULL );
    int         index( MStatus * ReturnStatus = NULL ) const;

	OPENMAYA_DEPRECATED(2019, "Use MItMeshEdge::currentItem instead.")
