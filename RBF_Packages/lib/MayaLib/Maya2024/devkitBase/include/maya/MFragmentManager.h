...
public:
	...
	//! Specifies shader stage in GPU pipeline.
	OPENMAYA_ENUM(ShaderStage,
		//! Vertex shader
		kVertexShader,
		//! Pixel shader
		kPixelShader,
		//! Geometry shader
		kGeometryShader,
		//! Pre-tessellation hull shader
		kHullShader,
		//! Pre-tessellation hull constant shader
		kHullConstantShader,
		//! Post-tessellation domain shader
		kDomainShader
	);
	...
	OPENMAYA_AVAILABLE(2022)
	bool addAutomaticShaderStageInput(
		ShaderStage shaderStage,
		const MString& parameterName,
		const MString& semantic,
		MShaderInstance::ParameterType parameterType,
		bool isVaryingInput);
	OPENMAYA_AVAILABLE(2022)
	bool removeAutomaticShaderStageInput(
		ShaderStage shaderStage,
		const MString& parameterName);

	OPENMAYA_AVAILABLE(2022)
	bool addDomainShaderInputNameMapping(
		const MString& realParamName,
		const MString& domainParamName);
	OPENMAYA_AVAILABLE(2022)
	...
	MStatus getColorManagementFragmentInfo(const MString& inputColorSpace, MString& fragmentName, MString& inputName, MString& outputName);
